...
final class PreferenceStorage {
    ...
    private Preferences[] getReadPreferences() {
        final var readPreferences = new Preferences[m_readContexts.length];
        for (var i = 0; i < m_readContexts.length; i++) {
    ...
    private <T> void writeValue(final String key, final T value) {
        ...
        try {
            ...
            NodeLogger.getLogger(PreferenceStorage.class)
    ...
    private <T> T readValue(final String key, final T defaultValue, final Function<String, T> fromString) {
        ...
        final var stringValue = Platform.getPreferencesService().get(key,
