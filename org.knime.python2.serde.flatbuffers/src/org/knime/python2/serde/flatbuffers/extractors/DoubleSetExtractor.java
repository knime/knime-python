 ...
 */
public class DoubleSetExtractor implements VectorExtractor {

    private DoubleCollectionColumn m_colVec;
     ...
     */
    public DoubleSetExtractor(final DoubleCollectionColumn colVec) {
        m_colVec = colVec;
    }
    ...
    public Cell extract() {
        ...
        final DoubleCollectionCell cell = m_colVec.values(m_ctr);

        final double[] values;
        if (cell.keepDummy()) {
            values = new double[cell.valueLength() + 1];
        } else {
            values = new double[cell.valueLength()];
        }
        cell.valueAsByteBuffer().asDoubleBuffer().get(values, 0, cell.valueLength());
        Double[] l = ArrayUtils.toObject(values);
        if (cell.keepDummy()) {
            l[l.length - 1] = null;
        }
        m_ctr++;
