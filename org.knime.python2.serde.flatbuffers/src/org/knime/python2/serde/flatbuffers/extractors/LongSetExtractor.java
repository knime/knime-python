 ...
 */
public class LongSetExtractor implements VectorExtractor {

    private LongCollectionColumn m_colVec;
     ...
     */
    public LongSetExtractor(final LongCollectionColumn colVec) {
        m_colVec = colVec;
    }
    ...
    public Cell extract() {
        ...
        final LongCollectionCell cell = m_colVec.values(m_ctr);

        final long[] values;
        if (cell.keepDummy()) {
            values = new long[cell.valueLength() + 1];
        } else {
            values = new long[cell.valueLength()];
        }
        cell.valueAsByteBuffer().asLongBuffer().get(values, 0, cell.valueLength());
        Long[] l = ArrayUtils.toObject(values);
        if (cell.keepDummy()) {
            l[l.length - 1] = null;
        }
        m_ctr++;
