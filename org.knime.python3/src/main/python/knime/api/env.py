...
def _set_paths():
    ...
    if python_bin_dir not in path_entries:
        ...
        new_path = pathsep_join(current_path, python_bin_dir)
    ...
    if os.path.exists(lib_dir):
        ...
        if system == "linux":
            ...
            os.environ["LD_LIBRARY_PATH"] = pathsep_join(current_ld_path, lib_dir)

        elif system == "darwin":  # macOS
            # Set DYLD_FALLBACK_LIBRARY_PATH on macOS
            current_dyld_path = os.environ.get("DYLD_FALLBACK_LIBRARY_PATH", "")
            os.environ["DYLD_FALLBACK_LIBRARY_PATH"] = pathsep_join(
