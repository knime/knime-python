...
public final class PortObjectConverters {
    ...
    public static final class TablePortObjectConverter implements PortObjectConverter<BufferedDataTable, PythonTablePortObject, PurePythonTablePortObject> {

        @Override
        public PythonTablePortObject toPython(final BufferedDataTable portObject, final ConversionContext context) {
            return new PythonTablePortObject(portObject, context.tableConverter());
        }

        @Override
        public PythonTablePortObject fromPython(final PurePythonTablePortObject pythonPortObject, final ConversionContext context) {
            try {
                PythonArrowDataSink sink = pythonPortObject.getPythonArrowDataSink();
                var bdt = context.tableConverter().convertToTable(sink, context.execContext());
                return new PythonTablePortObject(bdt, context.tableConverter());
