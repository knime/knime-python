 ...
 *
 * History
 *   Mar 20, 2024 (Adrian Nembach, KNIME GmbH, Konstanz, Germany): created
 */
...
public final class PythonHubAuthenticationPortObjectSpec extends PythonCredentialPortObjectSpec {
    ...
    public PythonHubAuthenticationPortObjectSpec(final AbstractHubAuthenticationPortObjectSpec spec) {
        super((CredentialPortObjectSpec)spec);
        m_spec = spec;
    }

    @Override
    public String getJavaClassName() {
        return AbstractHubAuthenticationPortObjectSpec.class.getName();
    }

    @Override
    protected void fill(final ObjectNode node) {
        super.fill(node);
        node.put("hub_url", m_spec.getHubURL().map(URI::toString).orElse(null));
    }
}
