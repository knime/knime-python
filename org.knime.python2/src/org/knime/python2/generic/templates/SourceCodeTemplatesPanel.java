...
public class SourceCodeTemplatesPanel extends JPanel {
    ...
    private final SourceCodeTemplateRepository m_repository;
    ...
    private final JButton m_remove;

    private final JComboBox<String> m_category;

    private final DefaultComboBoxModel<String> m_categoryModel;

    private final JList<SourceCodeTemplate> m_template;

    private final JTextArea m_description;

    private final DefaultListModel<SourceCodeTemplate> m_templateModel;

    /**
     * Constructor.
     *
     * @param sourceCodePanel the associated source code panel
     * @param repositoryId the unique name of the {@link SourceCodeTemplateRepository} containing the
     *            {@link SourceCodeTemplate}s
     */
    public SourceCodeTemplatesPanel(final SourceCodePanel sourceCodePanel, final String repositoryId) {
        ...
        final int maxWidth = Math.max(Math.max(m_create.getPreferredSize().width, m_apply.getPreferredSize().width),
            m_remove.getPreferredSize().width);
        final int maxHeight = Math.max(Math.max(m_create.getPreferredSize().height, m_apply.getPreferredSize().height),
    ...
    private void refreshCategories() {
        final String selected = (String)m_category.getSelectedItem();
        m_categoryModel.removeAllElements();
        final Set<String> categories = m_repository.getCategories();
    ...
    private void refreshTemplates() {
        final SourceCodeTemplate selected = m_template.getSelectedValue();
        m_templateModel.removeAllElements();
        final String category = (String)m_category.getSelectedItem();
        final Set<SourceCodeTemplate> templates = m_repository.getTemplatesForCategory(category);
        for (final SourceCodeTemplate template : templates) {
    ...
    private void refreshTemplateInfo() {
        final SourceCodeTemplate template = m_template.getSelectedValue();
        if (template != null) {
    ...
    private void openCreateDialog() {
        ...
        final JComboBox<String> category = new JComboBox<String>(m_repository.getCategories().toArray(new String[0]));
        ...
        if (apply.get()) {
            try {
                m_repository.createTemplate((String)category.getSelectedItem(), title.getText(), description.getText(),
                    m_sourceCodePanel.getEditor().getText());
                refreshCategories();
            } catch (IOException | IllegalArgumentException e) {
                JOptionPane.showMessageDialog(this, e.getMessage(), "Could not create template",
