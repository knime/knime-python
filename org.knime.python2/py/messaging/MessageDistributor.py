...
class MessageDistributor(object):
    ...
    def register_message_handler(self, message_category, handler):
        if message_category not in self._message_handlers:
            self._message_handlers[message_category] = handler
            return True
        else:
            return False

    def unregister_message_handler(self, message_category):
        if message_category in self._message_handlers:
            del self._message_handlers[message_category]
            return True
        else:
            return False

    def can_handle(self, message_category):
        return message_category in self._message_handlers
    ...
    def handle(self, message):
        ...
        if message_handler is not None:
            ...
            raise RuntimeError(
                "Message '" + str(
                    message) + "' cannot be distributed. No matching handler available for category '"
                + message.category + "'.")
