...
/**
 * Test transfer of different Arrow types to Python. Always tranfers one table with one column of a specific type.
 *
 * @author Benjamin Wilhelm, KNIME GmbH, Konstanz, Germany
 */
public class JavaToPythonTypeTest {

    // TODO test all with ArrowBatchStore?
    ...
    private <T extends NullableWriteData> void test(final String type, final DataSpec spec,
        ...
        try (final var store = createReadStore2(spec, valueSetter)) {
    ...
    /** Create a store filled using the given consumer */
    private <T extends NullableWriteData> ArrowBatchReadStore createReadStore2(final DataSpec spec,
        final ValueSetter<T> valueSetter) throws IOException {
        ...
        try (final var writeStore = m_storeFactory.createStore(schema, writePath)) {
            // Write some data to the store
            writeBatches(valueSetter, writeStore);
            // Move the file to the read store location
            Files.copy(writePath, readPath, StandardCopyOption.REPLACE_EXISTING);
        }
        return m_storeFactory.createReadStore(schema, readPath);
    }
    ...
    private static <T extends NullableWriteData> void writeBatches(final ValueSetter<T> valueSetter,
        final ArrowBatchStore writeStore) throws IOException {
